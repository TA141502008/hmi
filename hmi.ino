...
Summary        : 1. Initialize all component.
                 2. Do counting every second, reset every 3600 seconds.M
...
v1.3  1. Create a title in file.
      ...
      3. (not complete documentation)
v1.4  1. Reassign the rows in declaration section of source code
      2. Complete the source code with comments
	  3. Adding new procedure:
	     a. procedure_LED_SystemStatus_OpenFile_Error()
		 b. procedure_Relay_SETUP()
		 c. procedure_Relay_CH2_Alarm_ON()
		 d. procedure_ConditionCheck_Flag_ALL_temp()
		 e. procedure_ConditionCheck_Flag_Comparation()
		 f. procedure_GSM_SETUP()
		 g. procedure_GSM_SendMessage()
	  4. Editing some procedure:
         a. procedure_LED_SystemStatus_SETUP_All()
		 b. procedure_SensorData_ConditionCheck()
	  5. Editing SETUP section:
	     a. adding procedure_LED_SystemStatus_SETUP_All();
		 b. adding procedure_ConditionCheck_Flag_ALL_temp();
		 c. adding procedure_GSM_SETUP();
      6. Editing MAIN LOOP section:
	     a. adding procedure_ConditionCheck_Flag_Comparation(
		             ConditionCheck_Flag_ALL,
					 ConditionCheck_Flag_ALL_temp);
		 b. adding procedure_Relay_CH2_Alarm_ON();
		 c. adding procedure_GSM_SendMessage();
		 d. rearrange RELAY and GSM MODULE section to be last section
      7. Editing PROCEDURE DETAIL section:
	     a. Re-arrange the procedure
		 b. Add title depend on each category
...
// status of Condition Check --- OK when NORMAL, xx when NOT NORMAL
...
// flag for ERROR Condition Check --- 1 when ERROR, 0 when NOT error
...
void loop()
{
  ...
  if ((tm.Hour == 0) && (tm.Minute == 0) && (tm.Second == 0))                               // when value of RTC_ThisDay variable equal with RTC_NextDay variable
  {
    procedure_SDCard_RemoveOldFile();                           // REMOVE file one year before
    procedure_SDCard_NewFile();                                 // create new file that named with YYYYMMDD of today
    RTC_NextDay += 1;                                           // add value of RTC_NextDay variable
...
//            3. status of each parameter
{
  ...
  if ((LCD_SensorData_Display == 2) && (MainCounter % 6 == 4))                   // when MainCounter value mod 6 equal 4
  {
    ...
    lcd.print("Dalam =");                                                        // display text "Dalam =" on LCD
...
// process : ...
{
  ...
  if (flagSendMessage)
  {
    ...
    sprintf(GSM_SMSText, "%s\nParameter Kolam(%d)\nSuhu = %s (%d)\npH = %s(%d)\nDO = %s(%d)\nKeruh = %s(%d)\nGaram = %s(%d)\nDalam = %s(%d)",
            ...
            Char_SensorData_Depth, ConditionCheck_Flag_Depth);
...
//           2. compare those value with standard value (SNI)
{
  ...
  if ((Value_SensorData_Depth >= Depth_Low) && (Value_SensorData_Depth <= Depth_High))
  {
    ...
    ConditionCheck_Status_Depth = "OK";           // when Depth NORMAL
